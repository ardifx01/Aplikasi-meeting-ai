...
import { saveBookingData, getLastSuggestions } from '../services/aiDatabaseService';
...
const AiAssistantPage: React.FC<AiAssistantPageProps> = ({ onNavigate, onBookingConfirmed, onAiBookingData }) => {
    ...
    const sendMessage = async (text: string) => {
        ...
        if (result.finalBooking) {
            ...
            try {
                ...
                const duration = 60;
                const ok = await saveBookingData(
                    userId,
                    sessionId,
                    BookingState.BOOKED,
                    {
                        roomName: result.updatedBookingData.roomName || result.finalBooking.roomName,
                ...
                if (!ok) {
                    console.warn('Peringatan: gagal menyimpan booking AI ke backend. Melanjutkan ke konfirmasi.');
                }
            } catch (e) {
                console.error('Gagal menyimpan booking AI:', e); // lanjutkan navigasi
            }
    ...
    const handleOneShotIfPossible = async (text: string): Promise<boolean> => {
        ...
        try {
            ...
            const meetingType = lower.includes('client') || lower.includes('klien') ? 'external' : 'internal';
            ...
            const ok = await saveBookingData(userId, sessionId, BookingState.BOOKED, {
